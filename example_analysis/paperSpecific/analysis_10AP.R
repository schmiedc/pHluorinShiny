...
# ==============================================================================
#
#  DESCRIPTION: Analyse of 10AP movie
#              
...
indir = "/data1/FMP_Docs/Projects/Publication_SynActJ/10AP_output_10AP-1/"

outputDirectory = "/data1/FMP_Docs/Projects/Publication_SynActJ/R10AP_output_10AP-1/"
...
table.signal_mean_before <- table.signal_mean_filter %>% filter(time >= 6 & time <= 9)
...
table.signal_mean_after <- table.signal_mean_filter %>% filter(time >= 10 & time <= 14)
...
# ------------------------------------------------------------------------------
# compute peak 
filtered_signal_sd_restrict <- filtered_signal_sd %>% filter(time < 30)

peaks <- filtered_signal_sd_restrict %>% group_by(name, roi) %>% dplyr::summarize(value = max(value))
peaks_frame <- left_join(peaks, table.signal_mean_filter, by = c("name", "roi", "value"))

# filter for peak occurance 
filtered_peaks <- peaks_frame %>% filter(time >= peak_filter_lower && time <= peak_filter_upper)
...
ggsave(plot = raw_signal_grids,
       file=file.path(outputDirectory, paste0(resultname, "_keep_sd", sd_multiplicator,"_Lower", peak_filter_lower, "_Upper", peak_filter_upper,".pdf") ), 
       width = 297, 
       height = 210, 
       units = "mm") 
...
# save rejected
# filter for peak occurance 
filtered_peaks_reject <- filtered_signal_sd_restrict %>% filter(time <= peak_filter_lower && time >= peak_filter_upper)
...
ggsave(plot = raw_signal_grids_reject,
       file=file.path(outputDirectory, paste0(resultname, "_reject_Lower", peak_filter_lower, "_Upper", peak_filter_upper,".pdf") ), 
       width = 297, 
       height = 210, 
       units = "mm") 
