...
collectList <- function(inputdir, label, timeRes){
  ...
  combine.table2 <- plyr::rename(combine.table2, c("name" = "name", 
                           "X" = "frame", 
                           "roi" = "roi",
                           "Area1" = "area", 
                           "Mean1" = "mean",
...
calcBoutonNumber <- function(inputTable){
  ...
  boutonsNumbers <-plyr::rename(boutonsNumbers, c(".id"="name"))
  boutonsNumbers <-plyr::rename(boutonsNumbers, c("X..i.."="number_segments"))
